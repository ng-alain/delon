...
export interface Lodop extends CLodop {
     ...
     *
     * @param {string} strTaskName 打印任务名
     * @returns {boolean} 返回逻辑真表示初始化成功，逻辑假表示初始化失败，失败原因有：前一个打印事务没有完成；操作系统没有添加打印机(驱动)等
     */
     ...
     * 设置软件产品注册信息
     *
     * @param {string} strCompanyName 注册单位名称，用途与控件参数CompanyName一样。
     * @param {string} strLicense 主注册号，用途与控件参数License一样。
     * @param {string?} strLicenseA 附加注册号A，用途与控件参数LicenseA一样。
     * @param {string} [strLicenseB] 附加注册号B，用途与控件参数LicenseB一样。
     */
...
export type LodopStyleValue = 'FontName' | 'FontSize' | 'FontColor' | 'Bold' | 'Italic' | 'Underline' |
                              'Alignment' | 'Angle' | 'ItemType' | 'HOrient' | 'VOrient' | 'PenWidth' |
                              'PenStyle' | 'Stretch' | 'PreviewOnly' | 'ReadOnly';
