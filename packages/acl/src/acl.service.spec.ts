...
describe('acl: service', () => {
  ...
  describe('#can()', () => {
    it('should working', () => {
      ...
      expect(srv.can(ADMIN), `can ${ADMIN}`).toBe(true);
      expect(srv.can(ABILITY_NUMBER), 'ability muse be true').toBe(true);
      expect(srv.can([ABILITY_NUMBER]), 'ability array muse be true').toBe(true);
      expect(srv.can([ADMIN]), 'role array muse be true').toBe(true);
      expect(srv.can({ role: [ADMIN] } as ACLType), 'ACLType item muse be true').toBe(true);
  ...
  it(`#canAbility()`, () => {
    srv.attachAbility([ABILITY]);
    expect(srv.canAbility(ABILITY), 'should be support number or string type').toBe(true);
    expect(srv.canAbility([ABILITY]), 'should be support array type').toBe(true);
    expect(srv.canAbility(`${ADMIN}1`), 'should be invalid ability').toBe(false);
  });
  ...
  it('#change', async () => {
    const res = (await firstValueFrom(srv.change)) as ACLType;
