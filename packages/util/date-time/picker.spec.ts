...
describe('util: date-time', () => {
  ...
  describe('#dateTimePickerUtil', () => {
    ...
    it('#disabledBeforeDate', () => {
      expect(dateTimePickerUtil.disabledBeforeDate()(NOW), `Shoule be not include today`).toBe(false);
      expect(
        dateTimePickerUtil.disabledBeforeDate({ offsetDays: 1 })(NOW),
        `Shoule be not today, because of offset`
      ).toBe(true);
    });
    it('#disabledAfterDate', () => {
      expect(dateTimePickerUtil.disabledAfterDate()(NOW), `Shoule be not include today`).toBe(false);
      expect(
        dateTimePickerUtil.disabledAfterDate({ offsetDays: -1 })(NOW),
        `Shoule be not today, because of offset`
      ).toBe(true);
    });
