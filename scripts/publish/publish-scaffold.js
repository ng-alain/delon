...
const nextVersion = fs.readJSONSync(path.join(__dirname, '../../package.json')).version;
...
function fetchOlderVersions() {
  ...
  execSync('git checkout 10.x', execSyncOptions);
  execSync('git pull upstream 10.x', execSyncOptions);
  execSync('git fetch upstream 10.x --prune --tags', execSyncOptions);
...
function checkout() {
  ...
  execSync(`git commit -m "release(${nextVersion}): release ${nextVersion}"`, execSyncOptions);
