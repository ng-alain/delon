...
import { SiteConfig, ModuleConfig, Meta, MetaTemplateData, ModuleTemplateData, MetaOriginal, ContentTemplateData } from './interfaces';
...
function generateModule(config: ModuleConfig) {
    ...
    config.dir.forEach(dirConfig => {
        ...
        const files = groupFiles(
            dirConfig.src.map(p => path.join(rootDir, p)), dirConfig, isSyncSpecific, target, siteConfig);
        ...
        files.forEach(item => {
            ...
            generateDoc(
                fileObject,
                tpl,
                path.join(distPath, item.key, `index.ts`)
            );
            // endregion
